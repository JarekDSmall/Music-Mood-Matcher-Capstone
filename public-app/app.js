import {discover,savedTracks} from './discovery.mjs';
...
let recent=[];try{recent=JSON.parse(sessionStorage.getItem('mmm.recent')||'[]');if(!Array.isArray(recent))recent=[];}catch{}
...
function saveDraft(){try{sessionStorage.setItem(DRAFT,JSON.stringify({tracks,trackIds:tracks.map(t=>t.id),name:$('playlist-name').value,...selection()}));}catch{}}
function invalidateReview(){resolved=[];createdPlaylist=null;$('spotify-review').hidden=true;$('spotify-result').hidden=true;}
function setBusy(value){busy=value;for(const id of ['prepare','export','connect','disconnect','match','reset','shuffle','save'])$(id).disabled=value;}
...
function render(){
 $('tracks').replaceChildren();tracks.forEach((track,index)=>{
  ...
  const actions=node('div',undefined,'track-tools'),link=node('a','Open in Spotify ↗');link.href=track.uri?'https://open.spotify.com/track/'+track.id:'https://open.spotify.com/search/'+encodeURIComponent(track.title+' '+track.artist);link.target='_blank';link.rel='noopener noreferrer';link.setAttribute('aria-label',`Find ${track.title} by ${track.artist} on Spotify`);
...
async function generate(){
 if(busy)return;
 if(!spotify.connected()){message('Connect Spotify to discover 10 fresh tracks. Approved test accounts can use this app.');$('connect').focus();return;}
 const prefs=selection();setBusy(true);message('Searching Spotify for fresh tracks…');
 try{
  const fresh=await discover(spotify.api,prefs,recent);
  tracks=fresh;recent=[...recent,...fresh.map(t=>t.id)].slice(-200);try{sessionStorage.setItem('mmm.recent',JSON.stringify(recent));}catch{}
  $('playlist-heading').textContent=moods[prefs.mood];$('playlist-name').value=moods[prefs.mood];
  $('match-description').textContent='Live Spotify search · Mood and energy guide search terms; these are not measured audio features.';
  invalidateReview();render();message(fresh.length===10?'10 fresh tracks from Spotify. Find another 10 whenever you want.':`${fresh.length} fresh tracks found. Try another mood or broader sound preference for more; recent tracks were excluded.`);
 }catch(e){message(e.message);updateConnection();}finally{setBusy(false);render();}
}
function readSaved(){try{const raw=JSON.parse(localStorage.getItem(KEY)||'[]');saved=Array.isArray(raw)?raw.filter(p=>p&&typeof p.id==='string'&&typeof p.name==='string').slice(0,20).map(p=>({...p,name:p.name.slice(0,80),tracks:savedTracks(p.tracks),trackIds:Array.isArray(p.trackIds)?p.trackIds:[]})):[];}catch{saved=[];}}
...
function renderSaved(){
 ...
 saved.forEach(p=>{const row=node('div',undefined,'saved-item'),info=node('div');info.append(node('div',p.name,'saved-name'),node('small',`${p.tracks.length||p.trackIds.length} tracks`));const actions=node('div',undefined,'saved-actions');
 const load=node('button','Load','quiet');load.setAttribute('aria-label',`Load ${p.name}`);load.onclick=()=>{tracks=p.tracks.length?p.tracks:p.trackIds.map(id=>catalog.find(t=>t.id===id)).filter(Boolean);$('playlist-name').value=p.name;$('playlist-heading').textContent=p.name;$('match-description').textContent='From your saved collection.';invalidateReview();render();message(`Loaded ${p.name}.`);};
...
$('save').onclick=()=>{if(!tracks.length)return;const name=$('playlist-name').value.trim()||'My mood playlist';if(saved.length>=20){message('You have 20 saved playlists. Delete one before saving another.');return;}if(persistSaved([{id:crypto.randomUUID(),name,tracks,trackIds:tracks.map(t=>t.id)},...saved]))message(`Saved ${name} on this device.`);};
...
$('prepare').onclick=async()=>{
 ...
 setBusy(true);invalidateReview();const batch=[...tracks];spotifyMessage('Preparing your Spotify tracks for review…');
 try{
  const matches=[];for(const t of batch){if(t.uri){matches.push({track:t,match:{uri:t.uri}});continue;}const q=`track:${t.title} artist:${t.artist}`;const result=await spotify.api('search?'+new URLSearchParams({q,type:'track',limit:'5'}));matches.push({track:t,match:exactSpotifyMatch(t,result.tracks?.items||[])});}
...
$('export').onclick=async()=>{
 ...
 try{
  const playlist=await spotify.api('me/playlists',{method:'POST',body:JSON.stringify({name:$('playlist-name').value.trim()||'My mood playlist',public:false,description:'Mood-inspired Spotify discoveries from Music Mood Matcher by Jarek Small.'})});
...
readSaved();renderSaved();render();message(spotify.connected()?'Choose your mood, then find 10 fresh Spotify tracks.':'Connect Spotify to discover 10 fresh tracks.');
try{const draft=JSON.parse(returnDraft||'null');if(draft&&Array.isArray(draft.trackIds)&&new URLSearchParams(location.search).has('code')){tracks=savedTracks(draft.tracks);if(!tracks.length)tracks=draft.trackIds.map(id=>catalog.find(t=>t.id===id)).filter(Boolean);if(moods[draft.mood])document.querySelector(`input[value="${draft.mood}"]`).checked=true;if(Number.isInteger(draft.energy)&&draft.energy>=1&&draft.energy<=5)$('energy').value=draft.energy;if(['all','pop','indie','soul','electronic'].includes(draft.genre))$('genre').value=draft.genre;$('energy-value').textContent=names[Number($('energy').value)];$('playlist-name').value=String(draft.name||'My mood playlist').slice(0,80);render();}}catch{}
updateConnection();spotify.finishAuthorization().then(done=>{updateConnection();if(done){spotifyMessage('Spotify connected. Discovering 10 fresh tracks…');generate();}}).catch(e=>{spotifyMessage(e.message);updateConnection();});
